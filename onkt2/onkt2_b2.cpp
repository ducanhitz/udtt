...
void SearchMax(HocSinh H[], int l, int r, float &max, int &p)
{
    float max1, max2;
    if (l == r)
    {
    ...
    else
    {
        ...
        max1 = l;
        max = mid + 1;
        SearchMax(H, l, mid, max1, p);
        SearchMax(H, mid + 1, r, max2, p);
        if (max1 > max2)
        {
            max = max1;
        }
...
int main()
{
    ...
    //
    cout <<"\n------------------------"<<endl;
    cout << "Do dai xau con lon nhat: " << algo(H[0].tenHS, H[n-1].tenHS) << endl;
    Xau_con(H[0].tenHS, H[n-1].tenHS);
